...
;Para darle un tiempo en alto, mueve el servo a 90 grados
sw1pressed
		ADD R8, #1  					;conteo del servio
		LDR	R1, =LEDS
		MOV R0, #5
		STR	R0, [R1]
...
delay_off1
		...
		BL  delay1						; delay del al menos (3*R0) ciclos
		CMP R8, #18						
		BEQ Reinicio_r8 				; Se reinicia cuando llega a 18
		B sw1pressed

;Conteo de las cajasA
Reinicio_r8
		LDR R8, =1		;se le asigna el valor de uno y se le suma 1
		ADD R9, #1
		CMP R9, #1
		BEQ CajaA		;Va comparando que numero de caja va pasando
		CMP R9, #2
		BEQ CajaA1		;Si es uno va para la Caja1
		CMP R9, #3
		BEQ CajaA2
		CMP R9, #4
		BEQ CajaA3
...
sw2pressed
		ADD R8, #1						;Para que regrese a su posicion original
		LDR	R1, =LEDS
		MOV R0, #5
		STR	R0, [R1]
		LDR R10, =5333	               	; 5333333/1000 = 1 mili segundo aprox //5333 periodo para que se meuva el sservo
		BL  delay                       ; delay del al menos (3*R0) ciclos
		B   delay_off
...
delay_off
		...
		MOV R0, #6
		STR	R0, [R1]
		LDR R10, =101327              	; 5333 * 18 = 18 mili segundo aprox //101327
		BL  delay		; delay del al menos (3*R0) ciclos
		CMP R8, #20
		BEQ ReinicioR8
		B sw2pressed
		
;Conteo de las CajasB		
ReinicioR8
		LDR R8, =1
		;B Inicio
		ADD R3, #1
...
;Para encender un led de las cajas
CajaA
		LDR R0, =PA2				;00000100
